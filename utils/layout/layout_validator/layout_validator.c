...
static void validate_layout_size(const t_obj_type *const *buffer, t_xy temp, t_val_info **val_info)
{
	int mem_x;
...
static void validate_layout_walls(const t_obj_type *const *buffer, t_xy_size temp, t_val_info **val_info)
{
	...
	{
		if (buffer[0][temp.x] != WALL || buffer[(temp.height - 1)][temp.x] != WALL)
			(*val_info)->is_surrounded_with_walls = FALSE;
		temp.x++;
	}
	while (temp.y < (temp.height - 1))
	{
		if (buffer[temp.y][0] != WALL || buffer[temp.y][(temp.width - 1)] != WALL)
			(*val_info)->is_surrounded_with_walls = FALSE;
		temp.y++;
	}
}

static void count_objs(const t_obj_type *const *buffer, t_xy temp, t_val_info **val_info)
{
	while (buffer[temp.y])
	{
		temp.x = 0;
